...
jsonRequest.open("GET", "products.json");
jsonRequest.responseType = "json";

jsonRequest.onload = function () {
	if (this.status == 200) {
		products = jsonRequest.response;
		initialize();
	} else {
		console.log(
			"Network request for products.json failed with response " + jsonRequest.statusText
		);
	}
};
...
function initialize() {
	...
	// display it
	/**
	 * @param {{ name: string; price: number; image: string; type: string }} product
	 */
